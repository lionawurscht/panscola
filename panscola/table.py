...
class TableCell(pf.TableCell):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rm_horizontal_margins = utils.check_type(rm_horizontal_margins, bool)
...
def render_table(elem, doc):
    utils.count_elements(
        ...
        pf.Table,
        register="table",
        scope={"relevant_elems": pf.Header, "level": 2},
    )
    ...
    if isinstance(elem, pf.Div) and "custom_table" in elem.classes:
        ...
        if doc.format == "latex":
            ...
            return pf.RawBlock(render_table_odt(elem, doc), format="opendocument")
...
def render_table_footer_latex(elem, doc, tex):
    if isinstance(elem, pf.DefinitionList):
        ...
        with tex.create(
            lc.TableNotes(prefix="tn{}".format(".".join(table_number)))
        ) as tn:
...
def render_table_cell_latex(cell, c, top_space, btm_space, widths):
    ...
    if cell.col_span > 1:
        ...
        multicolumn = pl.MultiColumn(
            cell.col_span, align=pl.NoEscape(f"{margins}l{margins}"), data=cell_wrapper
        )
...
def render_table_latex(elem, doc):
    ...
    if hasattr(table, "caption") and table.caption:
        ...
        caption = ("\\caption{{{caption}}} \\\\ \\endfirsthead").format(caption=caption)

    table_spec = "@{{}} {columns} @{{}}".format(
        columns="".join(["p{{{}\\textwidth}}".format(w) for w in widths])
    )

    tex.append(pl.Command("renewcommand", arguments=[pl.Command("tabcolsep"), "0.3em"]))
    with tex.create(lc.myLongTable(table_spec, booktabs=True)) as lt:

        if hasattr(table, "caption") and table.caption:
            lt.append(
                pl.Command("caption", "".join(pf.stringify(c) for c in table.caption))
            )
            ...
            if row.underlines:
                for underline in row.underlines:
                    ...
                    if start == 1 and stop == table.col_cnt:
                        ...
                        lt.add_hline(start=start, end=stop, cmidruleoption=side)
...
def create_column_definitions(widths, table_name):
    for w, width in enumerate(widths):
        column_style_name = "{table_name}.{c}".format(
            table_name=table_name, c=utils.number_to_uppercase(w)
        )
    ...
    for r, row in enumerate(table.content):
        ...
        for c, cell in enumerate(row.content):
            ...
            if cell.covered:
                ...
                if cell.col_span > 1:
                    cell_odt.attrs["table:number-columns-spanned"] = cell.col_span

                if cell.content:
                    cell_content = utils.panflute2output(
                        cell.content, format="opendocument"
                    ).strip()

                    cell_content = BeautifulSoup(cell_content, "lxml").html.body
                        ...
                        if cell.vertical:
                            ...
                            t.contents = [
                                utils.create_nested_tags(
                                    **{
                                        "name": "text:span",
                                        "attrs": {"text:style-name": "Vertical"},
                                        "contents": t_contents,
                                    }
                                )
                            ]
                    ...
                    cell_content.attrs = {"text:style-name": "Table_20_contents"}
        ...
        if row.top_space or add_top_space:
            for cell_style in row_cell_styles:
                if cell_style is not None:
                    padding_top = cell_style.contents[0].attrs["fo:padding-top"]

                    padding_top = (
                        float(padding_top.strip("cm"))
                        + 0.05 * add_top_space
                        + 0.05 * row.top_space
                    )

                    cell_style.contents[0].attrs["fo:padding-top"] = f"{padding_top}cm"
    ...
    if footer is not None:
        for definition_item in footer.content:
            ...
            definition = utils.create_nested_tags(
                **{
                    "name": "text:p",
                    "attrs": {"text:style-name": "Table_20_Legend"},
                    "contents": [
                        {
                            "name": "text:span",
                            "attrs": {"text:style-name": "Superscript"},
                            "contents": [term],
                        }
                    ]
                    + definitions_parsed,
...
def _prepare(doc):
    ...
    custom_styles_root.append(
        utils.create_nested_tags(
            **{
                "name": "style:style",
                "attrs": {"style:name": "Vertical", "style:family": "text"},
                "contents": [
                    {
    ...
    for n in ["Illustration", "Table", "Text", "Drawing"]:
        ...
        t.attrs = {
            "text:display-outline-level": str(doc.get_metadata("outline-level", "1")),
            "text:name": n,
